...
import GridToolbar, { IToolbarAction, IToolbarActions } from "./grid-toolbar/GridToolbar";
...
const GridsView: FC<Omit<IGridsProps, "controller" | "dataHandler"> & IGridsViewProps> = ({
                                                                                            columns,
                                                                                            data,
                                                                                            vscode,
                                                                                            setGridsData,
  ...
  const [tableProps, changeTableProps] = useState(gridConfigurator.getTablePropsConfigurations());
  ...
  const defaultActionsHandler = new DefaultActionsHandler(tableProps, dispatch, vscode, setGridsData);
  ...
  const childComponents: ChildComponents = {
    cell: {
      elementAttributes: ({ rowKeyValue, column, rowKeyField }) => {
        ...
        const changedRow = changedItems.find(row => row[rowKeyField] === rowKeyValue);

        if (isCreatedRow(rowKeyValue)) {
          cellClassNames.push("created-row-cell");
        } else if (changedRow) {
          const initDataRow = initData.find(row => row[rowKeyField] === rowKeyValue);
  ...
  const defaultActions: Array<IToolbarAction> = [
    {
      icon: <PlusIcon />,
      callback: defaultActionsHandler.addNewRow(newRowData, tableProps.rowKeyField, setIsAdditionNewRow),
  ...
  const gridsHaveChanges = vscode.getState().changed.length > 0 || vscode.getState().newRows.length > 0 || vscode.getState().removedRows.length > 0;
  ...
  const actions: IToolbarActions = options.toolbarActions
    ? { defaultActions: [...options.toolbarActions, ...defaultActions], versionsControlActions }
