...
export class GridConfigurator {
  ...
  private readonly translator: TFunction<"translation", undefined, "translation">;

  constructor(options: IGridOptions, columns: Array<Column>, rowsData: RowsData, translator: TFunction<"translation", undefined, "translation">) {
  ...
  public useFinalValidation: CustomValidateFunc = (value, column) => {
    if (column.key !== ADDITION_COLUMN) {
      ...
      if (this.options.validation) {
        return (
          this.options.validation(value, column) || this.useDefaultValidation(value, column)
        );
      } else {
